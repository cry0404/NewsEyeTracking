...
INSERT INTO users (
    id, gender, age, date_of_birth, education_level, residence,
    weekly_reading_hours, primary_news_platform, is_active_searcher,
    is_colorblind, vision_status, is_vision_corrected
) VALUES (
    ...
    residence = COALESCE(EXCLUDED.residence, users.residence),
    weekly_reading_hours = COALESCE(EXCLUDED.weekly_reading_hours, users.weekly_reading_hours),
    primary_news_platform = COALESCE(EXCLUDED.primary_news_platform, users.primary_news_platform),
    is_active_searcher = COALESCE(EXCLUDED.is_active_searcher, users.is_active_searcher),
    is_colorblind = COALESCE(EXCLUDED.is_colorblind, users.is_colorblind),
    vision_status = COALESCE(EXCLUDED.vision_status, users.vision_status),
    is_vision_corrected = COALESCE(EXCLUDED.is_vision_corrected, users.is_vision_corrected)
RETURNING id, email, gender, age, date_of_birth, education_level, residence, weekly_reading_hours, primary_news_platform, is_active_searcher, is_colorblind, vision_status, is_vision_corrected, created_at
`
